...
/*用于控制器IP地址的宏定义*/
#define CONTROLLER_IP "192.168.58.25"
/******************************/
...
typedef struct _CTRL_STATE
{
	char       head[7];
	int        frame_len;               /* 消息帧长度                                     */
	float     runtime;                 /* 控制器启动时间,断电清零    */
	float 	   jt_tgt_pos[6];           /* 关节1-6目标位置                */
	float     jt_tgt_vel[6];           /* 关节1-6目标速度                */
	float     jt_tgt_acc[6];			/* 关节1-6目标加速度              */
	float     jt_tgt_cur[6];			/* 关节1-6目标电流		        */
	float 	   jt_tgt_tor[6];           /* 关节1-6目标扭矩                */
	float     jt_cur_pos[6];           /* 关节1-6当前位置                */
	float     jt_cur_vel[6]; 			/* 关节1-6当前速度                */
	float     jt_cur_cur[6];			/* 关节1-6当前电流 		        */
	float     jt_cur_tor[6];			/* 关节1-6当前扭矩                */
	float     tl_cur_pos[6];			/* 工具当前位置DKR		        */
	float	   tl_cur_vel[6];			/* 工具当前速度DKR                */
	float     tl_jtforce[6];			/**< 工具合力DKR			        */
	float     tl_tgt_pos[6];  			/**< 工具目标位置DKR	            */
	float     tl_tgt_vel[6];			/**< 工具目标速度DKR                */
	float     tl_cur_pos_base[6];      /**< 工具当前位置Base         */
	float     tl_tgt_pos_base[6];      /**< 工具目标位置Base          */
	...
	float     ctrl_time;               /**< 控制器实时线程执行时间         */
	float     test_data;               /**< 测试数据ur机器人软件使用       */
	uint8_t    robot_mode;              /**< 机器人模式                     */
	float     joint_mode[6];  			/**< 关节模式					    */
	float     safe_mode;               /**< 安全模式                       */
	int        collision_level;         /**< 碰撞等级,一级敏感,三级不敏感   */
	float     drag_enable;   			/**< 拖动使能                       */
	float     tl_acc[7];				/**< 工具加速度     			    */
	uint8_t    dr_com_err[6]; 			/**< 与驱动器通信故障               */
	float	   dr_err;                  /**< 驱动器哪个轴故障               */
	float     dr_alarm;                /**< 驱动器哪个轴警告               */
	float     out_sflimit_err;         /**< 超出软限位故障                 */
	float     collision_err;           /**< 碰撞故障                       */
	float     dr_err_code;  			/**< 驱动器故障代码 			    */
	float     dr_alarm_code;           /**< 驱动器警告代码                */
	float     vel_ratio;               /**< 机器人运行速度比例             */
	float     linear_m_bmak;           /**< 线性动量基准 				    */
	uint8_t    flag_zero_set;           /**< 机器人零位设定完成标志         */
	float     test_data1;  			/**< 测试数据ur机器人软件使用       */
	float     cb_vol;					/**< 控制板电压   				    */
	float     cb_robot_vol;            /**< 控制板机器人电压               */
	float     cb_robot_cur;            /**< 控制板机器人电流               */
	float     jt_cur_vol[6];           /**< 关节当前电压                   */
	...
	float     weldTrackSpeed;          /** 焊缝跟踪速度   mm/s */
	uint8_t    drag_alarm;              /** 拖动警告，当前处于自动模式,0-不报警，1-报警 ，2-位置反馈异常不切换 */
	float     LoadIdentifyData[4];     /** 负载辨识结果（weight,x,y,z） */
	int       conveyor_encoder_pos;    /** 传送带编码器位置 */
	float     conveyor_speed;          /** 传送带速度 mm/s */
	float     conveyorWorkPiecePos;    /** 传送带工件当前位置，单位mm */
	...
	float    tpd_record_scale;         /* TPD记录进度百分比，0~100 */
	uint8_t    tpd_exception;            /* TPD轨迹加载数量超限，0-未超限，1-超限 */
	float     FT_data[6];               /* 力/扭矩传感器数据,Fx,Fy,Fz,Tx,Ty,Tz */
	...
	float     flange_cur_pos[6];                     /** 末端法兰当前位姿 */
	...
	float robposCartTF[6];                           /* 对应工具工件下的笛卡尔位姿 */
	float robposJointTF[6];                          /* 对应工具工件下的关节位置 */
	...
	float descarj1[7];           						//关节1笛卡尔坐标位置
	float descarj2[7];          						//关节2笛卡尔坐标位置
	float descarj3[7];           						//关节3笛卡尔坐标位置
	float descarj4[7];          						//关节4笛卡尔坐标位置
	float descarj5[7];          						//关节5笛卡尔坐标位置
	float descarj6[7];           						//关节6笛卡尔坐标位置
	float desring1[7];           						//关节1限位环笛卡尔坐标位置及姿态
	float desring2[7];          						//关节2限位环笛卡尔坐标位置及姿态
	float desring3[7];           						//关节3限位环笛卡尔坐标位置及姿态
	float desring4[7];          						//关节4限位环笛卡尔坐标位置及姿态
	float desring5[7];          						//关节5限位环笛卡尔坐标位置及姿态
	float desring6[7];           						//关节6限位环笛卡尔坐标位置及姿态
	...
	float loadWeight;       							//web2.0负载重量
	float loadCoord[3];     							//web2.0负载质心
